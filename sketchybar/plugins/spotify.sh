...
update() {
  ...
  if [ "$SPOTIFY_DISPLAY_CONTROLS" = "true" ]; then
    ...
    shuffle=$(osascript -e 'tell application "Spotify" to get shuffling' 2>/dev/null || echo "false")
    repeat=$(osascript -e 'tell application "Spotify" to get repeating' 2>/dev/null || echo "false")
    
    sketchybar -m \
      --set spotify.shuffle icon.highlight=$([ "$shuffle" = "true" ] && echo "on" || echo "off") \
      --set spotify.repeat icon.highlight=$([ "$repeat" = "true" ] && echo "on" || echo "off") \
